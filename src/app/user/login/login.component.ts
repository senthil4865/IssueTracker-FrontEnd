...
export class LoginComponent implements OnInit {
  ...
  getErrorMessage() {
    this.email.hasError('required') ? 'You must enter a value' : this.email.hasError('email') ? 'Not a valid email' : '';
  }
    ...
    if (!this.email.value) {
      ...
      this.appService.signIn(data).subscribe((apiResponse) => {
        if (apiResponse.status == 200) {
          this.toastr.success("Login Successful", 'Welcome to Issue Tracker')
  ...
  public socialSignIn(socialPlatform: string) {
    ...
    this.socialService.signIn(socialPlatformProvider).then(
      (userData) => {

        if (userData.email && userData.name) {

          this.emailSocial = userData.email
